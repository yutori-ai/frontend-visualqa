...
def test_session_arg_help_mentions_persistent_singleton_and_run_name_scope() -> None:
    ...
    assert "Persistent mode supports one named session at a time." in verify_help
    assert "Optional label included in JSON output and reports." in verify_help
    assert "Persistent mode supports one named session at a time." in screenshot_help
    assert "Optional label included in JSON output." in screenshot_help

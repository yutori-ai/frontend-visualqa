...
# The DOM-inspection script run by capture_grounding_state(), kept as a real
# .js file (rather than an inline Python string) so it gets JS syntax
# highlighting/linting and reads like the browser-side script it is. Read
# once at import time; the module is only imported after the package is
# installed, so the sibling file is always present alongside this one.
_CAPTURE_GROUNDING_STATE_JS = (Path(__file__).parent / "capture_grounding_state.js").read_text(encoding="utf-8")
...
async def capture_grounding_state(session: BrowserSession) -> GroundingState:
    return await session.page.evaluate(_CAPTURE_GROUNDING_STATE_JS)

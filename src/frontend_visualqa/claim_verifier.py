...
class ClaimVerifier:
    ...
    async def verify(
        ...
        *,
        ...
        try:
            ...
            bootstrap_execution = await self._run_bootstrap_observation(
                session=session,
            )
            ...
            messages = [
                {
                    ...
                    "content": [
                        ...
                        *self._bootstrap_message_parts(bootstrap_execution, url=url, session=session),
                        {
                            "type": "image_url",
    ...
    async def _run_bootstrap_observation(
        self,
        *,
        session: BrowserSession,
    ) -> dict[str, Any] | None:
        tool_call = _SyntheticToolCall(
            function=_SyntheticFunction(name=BOOTSTRAP_TOOL_NAME, arguments="{}"),
        )
        try:
            return await self._execute_tool_call(session, tool_call)
        except Exception:
            logger.warning(
                "Bootstrap observation %s failed; continuing with screenshot-only seed",
                BOOTSTRAP_TOOL_NAME,
                exc_info=True,
            )
            return None
    ...
    @classmethod
    def _bootstrap_message_parts(
        cls,
        bootstrap_execution: dict[str, Any] | None,
        *,
        url: str,
        session: BrowserSession,
    ) -> list[dict[str, str]]:
        if bootstrap_execution is None:
            return []
        tool_result = cls._build_tool_result_text(
            trace=bootstrap_execution["trace"],
            output_text=bootstrap_execution.get("output_text"),
            current_url=bootstrap_execution.get("current_url", session.page.url) or url,
        )
        return [
            {
                "type": "text",
                "text": (
                    "Verifier-owned bootstrap observation. This preflight read was generated by the verifier, "
